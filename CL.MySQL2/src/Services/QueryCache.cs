...
public static class QueryCache
{
    ...
    /// <summary>How near to <c>UtcNow</c> a DateTime parameter must be to qualify for
    ...
    /// <summary>
    /// Default TTL applied by the parameterless <c>WithCache()</c> overloads, from
    ...
    /// <summary>
    /// Whether cache hit/miss observability events are published, from
    /// <see cref="Configuration.CacheConfiguration.PublishEvents"/> (default true).
    /// </summary>
    internal static bool PublishEvents { get; private set; } = true;

    // Per-connection overrides of the global switch, from each database's
    // MySqlDatabaseConfig.CacheEnabledOverride. Absent = inherit the global switch.
    ...
    /// of the global <see cref="Enabled"/> switch. Called by <c>MySQL2Library</c> at init.
    /// </summary>
    ...
    {
        if (connectionId is not null && _enabledOverrides.TryGetValue(connectionId, out var o))
            return o;
        return Enabled;
    }
