...
public sealed class TableSyncService
{
    ...
    /// <see cref="SqlServerDatabaseConfig.SchemaSyncLevel"/>. When null, sync operates at
    ...
    private static bool IsDestructive(string stmt) =>
        ...
        || stmt.Contains("DROP CONSTRAINT", StringComparison.OrdinalIgnoreCase)
        || stmt.Contains("ALTER COLUMN", StringComparison.OrdinalIgnoreCase);
        ...
        bool lockHeld = false)
    {
        ...
        try
        {
            ...
            async Task<bool> CanSkipAsync() =>
                ...
                && await TableExistsAsync(schemaName, tableName, connectionId, ct).ConfigureAwait(false);

            if (await CanSkipAsync().ConfigureAwait(false))
            {
                sw.Stop();
                _logger?.Debug($"[MSSQL] Table [{tableName}] unchanged (crc {modelCrc}) — skipped");
                return Result<SyncResult>.Success(SkipResult(modelCrc));
            }

            _logger?.Info($"[MSSQL] Syncing table [{tableName}] (mode: {mode}, level: {level}, crc {modelCrc})");
            ...
            if (!lockHeld)
            {
                ...
                if (!ownLock.Acquired)
                {
                    ...
                    sw.Stop();
                    _logger?.Warning($"[MSSQL] Skipped sync of [{tableName}] — another node holds the schema-sync lock.");
                    return Result<SyncResult>.Success(SkipResult(modelCrc));
                }
            ...
            try
            {
                var tableExists = await TableExistsAsync(schemaName, tableName, connectionId, ct).ConfigureAwait(false);
                ...
                else
                {
                    ...
                    if (createBackup || mode == SyncMode.Migration)
                    {
                        ...
                        {
                            _logger?.Warning($"[MSSQL] Schema backup failed for [{tableName}]: {backupResult.Error?.Message}");
                            if (mode == SyncMode.Migration)
                                throw new InvalidOperationException($"Migration mode requires a successful schema backup for [{tableName}].");
                        }
                    }
                    ...
                    foreach (var stmt in alterStatements)
                    {
                        ...
                        operations.Add(stmt);
                        _logger?.Debug($"[MSSQL] Applied: {stmt}");
                    }
                    ...
                    if (level < SchemaSyncLevel.Full)
                    {
                        ...
                        if (fullStatements.Any(IsDestructive))
                        {
                            ...
                            _logger?.Warning(
                                $"[MSSQL] Table [{tableName}]: destructive change(s) deferred under {mode} mode — run Migration mode to complete.");
                        }
                    }

                    if (operations.Count > 0)
                        _logger?.Info($"[MSSQL] Updated table [{tableName}] ({operations.Count} changes)");
                    else
                        _logger?.Debug($"[MSSQL] Table [{tableName}] is up to date");
                }

                // Record the new CRC + status — only after DDL succeeded. SQL Server auto-commits DDL,
        ...
        {
            sw.Stop();
            _logger?.Error($"[MSSQL] Table sync failed for [{tableName}]: {ex.Message}", ex);
            errors.Add(ex.Message);

            return Result<SyncResult>.Failure(
                Error.Internal("mssql.sync_failed", $"Table synchronization failed for {tableName}", ex.Message));
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        if (level != SchemaSyncLevel.None)
        {
            ...
            if (!lockHeld)
                _logger?.Warning("[MSSQL] Schema-sync pass could not obtain the lock — another node is syncing; unchanged tables will be skipped.");
        }
        ...
        try
        {
            ...
            foreach (var entityType in types)
            {
                ...
                {
                    _logger?.Error($"[MSSQL] Failed to sync [{tableName}]: {ex.Message}", ex);
            ...
            {
                _logger?.Warning(
                    "[MSSQL] Migration mode: schema already current — set SyncMode back to Production.");
            }
