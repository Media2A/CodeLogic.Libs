...
/// <summary>
/// Wraps a <see cref="SqlTransaction"/> with an async-disposable pattern.
...
public sealed class TransactionScope : IAsyncDisposable
{
    ...
    public async Task CommitAsync(CancellationToken ct = default)
    {
        ...
        _completed = true;
        _logger?.Info($"[MSSQL] Transaction committed for '{ConnectionId}'");
    }
    ...
    public async Task RollbackAsync(CancellationToken ct = default)
    {
        ...
        _completed = true;
        _logger?.Info($"[MSSQL] Transaction rolled back for '{ConnectionId}'");
    }
    ...
    public async ValueTask DisposeAsync()
    {
        if (!_completed)
        {
            try
            {
                ...
                _logger?.Warning($"[MSSQL] Transaction auto-rolled back on dispose for '{ConnectionId}'");
            }
            catch (Exception ex)
            {
                _logger?.Error($"[MSSQL] Error during auto-rollback for '{ConnectionId}': {ex.Message}", ex);
            }
            _completed = true;
        }
