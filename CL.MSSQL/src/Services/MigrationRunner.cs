...
public sealed class MigrationRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var m in _migrations)
        {
            ...
            {
                _logger?.Warning(
                    $"[MSSQL] Migration '{MigrationId(m)}' checksum changed since it was applied — its body may have been edited.");
            }
        }
        ...
        if (pending.Count == 0)
        {
            _logger?.Debug("[MSSQL] No pending migrations.");
        ...
        {
            _logger?.Warning("[MSSQL] Migration pass skipped — another node holds the schema-sync lock.");
            return Result<MigrationRunResult>.Success(new MigrationRunResult([], 0));
        }
        ...
        foreach (var m in pending)
        {
            ...
            try
            {
                ...
                _logger?.Info($"[MSSQL] Applied migration '{id}': {m.Description}");
            }
            catch (Exception ex)
            {
                _logger?.Error($"[MSSQL] Migration '{id}' failed: {ex.Message}", ex);
                return Result<MigrationRunResult>.Failure(
                    Error.Internal("mssql.migration_failed", $"Migration '{id}' failed", ex.Message));
            }
        }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            return Result<MigrationRunResult>.Failure(Error.Internal(
                "mssql.rollback_locked", "Rollback skipped — another node holds the schema-sync lock."));
        }
        ...
        foreach (var m in toRollback)
        {
            ...
            try
            {
                ...
                rolledBack.Add(id);
                _logger?.Info($"[MSSQL] Rolled back migration '{id}'");
            }
            catch (Exception ex)
            {
                _logger?.Error($"[MSSQL] Rollback of '{id}' failed: {ex.Message}", ex);
                return Result<MigrationRunResult>.Failure(
                    Error.Internal("mssql.rollback_failed", $"Rollback of '{id}' failed", ex.Message));
            }
        }
