...
public sealed class SmartCachePool : IAsyncDisposable
{
    ...
    internal void Start()
    {
        ...
        _logger?.Info($"[MSSQL] SmartCachePool '{Name}' started (refreshEvery={RefreshEvery.TotalSeconds:0.#}s).");
    }
    ...
    public void WarmUp(Func<Task> warmUp)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                sw.Stop();
                _logger?.Info($"[MSSQL] SmartCachePool '{Name}' warm-up complete: {_entries.Count} entries primed in {sw.ElapsedMilliseconds}ms.");
            }
            catch (Exception ex)
            {
                _logger?.Warning($"[MSSQL] SmartCachePool '{Name}' warm-up faulted: {ex.Message}");
            }
        });
    }
    ...
    private async Task LoopAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                _logger?.Warning($"[MSSQL] SmartCachePool '{Name}' tick faulted: {ex.Message}");
            }
    ...
    private async Task TickAsync(CancellationToken ct)
    {
        ...
        foreach (var kv in _entries)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger?.Warning($"[MSSQL] SmartCachePool '{Name}' refresh failed for {entry.TableName}/{entry.EntryId[..8]}: {ex.Message}");
            }
        }
    }
