...
{
    private readonly ConcurrentDictionary<string, Func<SqlDataReader, T>> _byShape = new();
    ...
    /// </summary>
    public Func<SqlDataReader, T> CompileForReader(SqlDataReader reader)
    {
        var shape = BuildShapeKey(reader);
        return _byShape.GetOrAdd(shape, _ => BuildMaterializer(reader));
    }

    private static string BuildShapeKey(SqlDataReader reader)
    {
    ...
    private static Func<SqlDataReader, T> BuildMaterializer(SqlDataReader reader)
    {
