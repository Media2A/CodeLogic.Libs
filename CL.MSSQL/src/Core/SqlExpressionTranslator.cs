...
/// <summary>
/// Translates column-level C# expressions into SQL Server SQL fragments. This is the
/// read-side counterpart to <see cref="SqlServerExpressionVisitor"/> (which handles WHERE
...
internal static class SqlExpressionTranslator
{
    ...
    private sealed class Visitor
    {
        ...
        private (string Sql, Type ClrType) VisitMember(MemberExpression m)
        {
            ...
            if (m.Expression is ParameterExpression pe && pe == _row)
            {
                ...
                var clrType = (m.Member as PropertyInfo)?.PropertyType
                              ...
                              ?? typeof(object);
                return (SqlServerDialect.Quote(colName), clrType);
            }

            // Captured closure member — evaluate to a literal. Primitives only for column
            // expressions; WHERE-side parameter binding is handled by SqlServerExpressionVisitor.
        ...
        private (string Sql, Type ClrType) VisitSqlFn(MethodCallExpression mc)
        {
            ...
            {
                case nameof(SqlFn.Year):       return ($"DATEPART(year, {Arg(0)})", typeof(int));
                case nameof(SqlFn.Month):      return ($"DATEPART(month, {Arg(0)})", typeof(int));
                case nameof(SqlFn.Day):        return ($"DATEPART(day, {Arg(0)})", typeof(int));
                case nameof(SqlFn.Hour):       return ($"DATEPART(hour, {Arg(0)})", typeof(int));
                case nameof(SqlFn.Minute):     return ($"DATEPART(minute, {Arg(0)})", typeof(int));
                case nameof(SqlFn.DayOfWeek):  return ($"((DATEDIFF(day, CONVERT(date, '19000107', 112), CONVERT(date, {Arg(0)})) % 7 + 7) % 7)", typeof(int));
                case nameof(SqlFn.Date):       return ($"CONVERT(date, {Arg(0)})", typeof(DateTime));
                case nameof(SqlFn.BucketUtc):  return ($"DATEADD(second, (DATEDIFF_BIG(second, CONVERT(datetime2, '19700101', 112), {Arg(0)}) / {Arg(1)}) * {Arg(1)}, CONVERT(datetime2, '19700101', 112))", typeof(DateTime));
                ...
                case nameof(SqlFn.IfNull):     return ($"COALESCE({Arg(0)}, {Arg(1)})", mc.Method.ReturnType);
        ...
        private (string Sql, Type ClrType) VisitAggregate(MethodCallExpression mc)
        {
            ...
            switch (method)
            {
                ...
                case nameof(Enumerable.Any) when mc.Arguments.Count == 1:
                    return ("CAST(CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS bit)", typeof(bool));
                ...
                case nameof(Enumerable.Any) when mc.Arguments.Count == 2:
                {
                    ...
                    var innerVisitor = new Visitor(lambda.Parameters[0], _group, _groupKey);
                    var predicate = innerVisitor.Visit(lambda.Body);
                    return ($"CAST(CASE WHEN SUM(CASE WHEN {predicate.Sql} THEN 1 ELSE 0 END) > 0 THEN 1 ELSE 0 END AS bit)", typeof(bool));
                }
