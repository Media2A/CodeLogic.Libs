...
/// <summary>
/// Translates LINQ lambda expressions into SQL Server WHERE clause fragments and parameter dictionaries.
/// Supports binary comparisons, logical operators, method calls (Contains, StartsWith, EndsWith),
/// and null checks.
/// </summary>
internal sealed class SqlServerExpressionVisitor : ExpressionVisitor
{
    private readonly StringBuilder _sql = new();
    private readonly Dictionary<string, object?> _parameters = new();
    private int _paramCounter;
    private int _largestInListCount;
    ...
    public SqlServerExpressionVisitor(string tableAlias = "")
    {
        _tableAlias = tableAlias;
    }

    private SqlServerExpressionVisitor(IReadOnlyDictionary<ParameterExpression, string> aliasMap)
    {
    ...
    public static (string Sql, Dictionary<string, object?> Parameters) Translate<T>(
        ...
        string tableAlias = "")
        => Translate(predicate, tableAlias, out _);

    /// <summary>
    /// Translates a predicate and reports the largest generated <c>IN (...)</c> list, so the
    /// caller can compare it with the configured <c>MaxInClauseValues</c> and warn. 0 when the
    /// predicate produced no IN list.
    /// </summary>
    public static (string Sql, Dictionary<string, object?> Parameters) Translate<T>(
        Expression<Func<T, bool>> predicate,
        string tableAlias,
        out int largestInListCount)
    {
        var visitor = new SqlServerExpressionVisitor(tableAlias);
        visitor.VisitPredicate(predicate.Body);
        largestInListCount = visitor._largestInListCount;
    ...
    {
        var visitor = new SqlServerExpressionVisitor(aliasMap);
        visitor.VisitPredicate(predicate.Body);
        return (visitor._sql.ToString(), visitor._parameters);
    }
    ...
    {
        if (node.NodeType is ExpressionType.AndAlso or ExpressionType.OrElse or ExpressionType.And or ExpressionType.Or)
        {
            _sql.Append('(');
            VisitPredicate(node.Left);
            _sql.Append(node.NodeType is ExpressionType.AndAlso or ExpressionType.And ? " AND " : " OR ");
    ...
    protected override Expression VisitUnary(UnaryExpression node)
    {
        ...
        {
            if (IsBooleanEntityMember(node.Operand))
            {
                _sql.Append('(');
                Visit(node.Operand);
                _sql.Append(" = 0)");
    ...
    {
        if (IsBooleanEntityMember(expression))
        {
            Visit(expression);
            _sql.Append(" = 1");
        }
    ...
    private static bool IsBooleanEntityMember(Expression expression)
    {
        while (expression is UnaryExpression { NodeType: ExpressionType.Convert or ExpressionType.ConvertChecked } unary)
            expression = unary.Operand;
        return expression is MemberExpression { Expression: ParameterExpression } && expression.Type == typeof(bool);
    }
    ...
    private string QualifyColumn(MemberInfo member, Expression? owner)
    {
        ...
        return string.IsNullOrEmpty(alias) ? SqlServerDialect.Quote(col) : $"{SqlServerDialect.Quote(alias)}.{SqlServerDialect.Quote(col)}";
    }
    ...
    private void EmitInClause(Expression collectionExpr, Expression itemExpr)
    {
        ...
        {
            if (items.Count > _largestInListCount) _largestInListCount = items.Count;
            Visit(itemExpr);
            _sql.Append(" IN (");
    ...
    private static object? EscapeLikeValue(object? value)
    {
        ...
        return SqlServerDialect.EscapeLike(s);
    }
}
