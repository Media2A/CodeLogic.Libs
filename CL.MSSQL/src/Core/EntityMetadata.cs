...
internal static class EntityMetadata<T> where T : class
{
    ...
    /// <summary>
    /// Compiled row materializer: reads a <see cref="SqlDataReader"/> positioned on a row
    ...
    static EntityMetadata()
    {
        ...
        SchemaName = TableAttr?.Schema ?? "dbo";
        QualifiedTableName = SqlServerDialect.Qualify(SchemaName, TableName);
...
internal sealed class ColumnMetadata
{
    ...
    public bool IsRowVersion => Attribute?.DataType == DataType.RowVersion;
