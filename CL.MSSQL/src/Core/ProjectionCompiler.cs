...
///   <item>a compiled <c>Func&lt;SqlDataReader, TResult&gt;</c> that pulls values out of
...
internal static class ProjectionCompiler
{
    ...
    public readonly struct Compiled<T, TResult>
    {
        ...
        public Func<SqlDataReader, TResult> Materializer { get; init; }
    }
    ...
    {
        var selectList = string.Join(", ", columns.Select(c => $"{c.Sql} AS {SqlServerDialect.Quote(c.Alias)}"));
    ...
    private static Compiled<T, TResult> Build<T, TResult>(Expression<Func<T, TResult>> projection)
    {
        ...
        var selectList = string.Join(", ",
            columns.Select(c => $"{c.Sql} AS {SqlServerDialect.Quote(c.Alias)}"));
    ...
    /// <summary>
    /// Translate a single column-level expression into a SQL Server SQL fragment and carry
    ...
    /// <summary>
    /// Build a delegate that reads aliased columns from <see cref="SqlDataReader"/> and
    ...
    /// </summary>
    private static Func<SqlDataReader, TResult> BuildMaterializer<TResult>(
        Expression body,
        List<(string Sql, string Alias, Type CLRType)> columns)
    {
