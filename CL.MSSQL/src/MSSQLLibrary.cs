...
/// <summary>
/// <b>CL.MSSQL</b> — CodeLogic library providing SQL Server database access with a fluent
/// LINQ query builder, automatic table synchronization, migrations, schema backups,
/// and multiple named database connections.
/// <para>
/// Lifecycle:
/// <list type="number">
///   <item><description><b>Configure</b> — Registers <see cref="DatabaseConfiguration"/> and <see cref="MSSQLStrings"/>.</description></item>
...
/// </para>
/// </summary>
public sealed class MSSQLLibrary : ILibrary
{
    /// <inheritdoc/>
    public LibraryManifest Manifest { get; } = new()
    {
        Id = "CL.MSSQL",
        Name = "SQL Server Library",
        Version = CL.Internal.InternalLibraryVersion.Current,
        Description = "SQL Server with typed LINQ, SQL aggregation, projection pushdown, compiled materializers, covering indexes, retention, and a working cache.",
        Author = "Media2A",
        Tags = ["mssql", "database", "orm", "repository"]
    };
    ...
    public Task OnConfigureAsync(LibraryContext context)
    {
        ...
        context.Configuration.Register<DatabaseConfiguration>("mssql");
        context.Configuration.Register<CacheConfiguration>("mssql.cache");
        context.Localization.Register<MSSQLStrings>();
    ...
    public async Task OnInitializeAsync(LibraryContext context)
    {
        ...
        _config = config;
        _strings = context.Localization.Get<MSSQLStrings>();
        ...
        foreach (var kvp in enabledDbs)
        {
            context.Logger.Info($"[MSSQL] Connecting '{kvp.Key}' to {kvp.Value.Host}:{kvp.Value.Port}/{kvp.Value.Database}");
        ...
        foreach (var kvp in enabledDbs)
        {
            context.Logger.Info($"[MSSQL] [{kvp.Key}] Sync mode: {kvp.Value.SyncMode}");
        ...
        QueryObservability.Configure(context.Events, context.Logger, _connectionManager,
            connectionId => config.Databases.TryGetValue(connectionId, out var database) ? database : null);

        context.Logger.Info(_strings?.LibraryInitialized ?? "SQL Server library initialized");
    }
    ...
    public async Task OnStartAsync(LibraryContext context)
    {
        ...
        try
        {
            foreach (var connectionId in _connectionManager.GetConnectionIds())
            {
                ...
                context.Logger.Info($"[MSSQL] [{connectionId}] Server: {serverInfo.Version} ({serverInfo.Comment})");
                context.Logger.Info($"[MSSQL] [{connectionId}] Database: {serverInfo.Database} on {serverInfo.Host}");
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task OnStopAsync()
    {
        ...
        _context?.Logger.Info(_strings?.LibraryStopped ?? "SQL Server library stopped");
    }
    ...
    {
        if (!_isEnabled)
            return HealthStatus.Healthy("SQL Server library is disabled");

        if (_connectionManager is null)
            return HealthStatus.Unhealthy("SQL Server library not initialized");
        ...
        {
            _context?.Logger.Error($"[MSSQL] Health check exception: {ex.Message}", ex);
    ...
    public Task<Result<List<T>>> SqlQueryAsync<T>(
        ...
        => ExecuteRawAsync(sql, parameters, connectionId, "mssql.query_failed", ct, async cmd =>
        {
    ...
    public Task<Result<int>> ExecuteSqlAsync(
        ...
        => ExecuteRawAsync(sql, parameters, connectionId, "mssql.execute_failed", ct,
    ...
    public Task<Result<T?>> SqlScalarAsync<T>(
        ...
        string connectionId = "Default",
        CancellationToken ct = default)
        => ExecuteRawAsync<T?>(sql, parameters, connectionId, "mssql.query_failed", ct, async cmd =>
        {
        ...
        Func<SqlCommand, Task<TOut>> run)
    {
        ...
        {
            _context?.Logger.Error($"[MSSQL] Raw SQL failed: {ex.Message} — {sql}", ex);
    ...
    public void SetSyncMode(SyncMode mode, string connectionId = "Default")
    {
        ...
        {
            cfg.SyncMode = mode;
            _context?.Logger.Info($"[MSSQL] [{connectionId}] Sync mode set to {mode} at runtime.");
        }
    }
    ...
    public MSSQLLibrary RegisterMigration(IMigration migration)
    {
    ...
    public MSSQLLibrary RegisterMigrationsFrom(System.Reflection.Assembly assembly)
    {
    ...
    /// <summary>
    /// Tests the SQL Server connection for the given connection ID.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        ...
        {
            return Result<bool>.Failure(Error.FromException(ex, "mssql.connection_failed"));
        }
    }
