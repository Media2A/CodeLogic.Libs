...
public sealed class CursorPaginationUnitTests
{
    ...
    public void Token_rejects_malformed_and_mismatched_ordering()
    {
        ...
        Assert.Equal("mssql.invalid_cursor",
    ...
    public void Token_rejects_oversized_input_before_decoding()
    {
        ...
        Assert.Equal("mssql.invalid_cursor", error.ErrorCode);
    ...
    [Fact]
    public void Seek_predicate_matches_sql_server_null_ordering()
    {
        ...
        Assert.Contains("[rank] = @cursor_0 OR ([rank] IS NULL AND @cursor_0 IS NULL)", ascSql);
        Assert.Null(ascParams["@cursor_0"]);
        ...
        Assert.Contains("[rank] = @cursor_0 OR ([rank] IS NULL AND @cursor_0 IS NULL)", descSql);
    }
