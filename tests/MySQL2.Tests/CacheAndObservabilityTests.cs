...
public sealed class QueryCacheFacadeTests
{
    ...
    public QueryCacheFacadeTests(MySQL2RuntimeFixture fx) => _fx = fx;
    ...
    // MySQL2Tests.CacheCoordinator_multiNode, which also asserts that a peer's broadcast
    // is not re-published (no echo loop). Not duplicated here.
...
public sealed class QueryObservabilityTests
{
    ...
    public QueryObservabilityTests(MySQL2RuntimeFixture fx) => _fx = fx;
    ...
    public async Task Each_recorder_publishes_its_event()
    {
        ...
        QueryObservability.RecordSlow("Default", "SELECT SLEEP(1)", 1500);
