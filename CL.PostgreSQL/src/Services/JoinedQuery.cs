    ...
    where TRight : class, new()
{
    ...
    private void AddPredicate(Expression body, IReadOnlyDictionary<ParameterExpression, string> aliasMap)
    {
        ...
        var (clause, parms) = PostgreSqlExpressionVisitor.TranslateMulti(lambda, aliasMap);
    ...
    public async Task<Result<List<TResult>>> ToListAsync(CancellationToken ct = default)
    {
        ...
        {
            _logger?.Error($"[PostgreSQL] JoinedQuery.ToListAsync failed: {ex.Message} — query: {DescribeForLog()}", ex);
    ...
    public async Task<Result<long>> CountAsync(CancellationToken ct = default)
    {
        ...
        {
            _logger?.Error($"[PostgreSQL] JoinedQuery.CountAsync failed: {ex.Message} — query: {DescribeForLog()}", ex);
    ...
    private string FromAndJoin() =>
        $"{PostgreSqlDialect.Quote(_leftTable)} AS {PostgreSqlDialect.Quote(LeftAlias)} {_joinKeyword} {PostgreSqlDialect.Quote(_rightTable)} AS {PostgreSqlDialect.Quote(RightAlias)} ON {_onClause}";
    ...
    private async Task<T> ExecuteAsync<T>(Func<NpgsqlConnection, Task<T>> action)
    {
