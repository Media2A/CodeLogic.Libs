...
/// executes on the migration's own connection and transaction (DDL excepted — PostgreSQL auto-commits
...
public interface IMigrationContext
{
    ...
    /// transform around it. Note: this issues DDL, which PostgreSQL commits immediately.
    /// </summary>
...
internal sealed class MigrationContext : IMigrationContext
{
    ...
    public NpgsqlConnection Connection => _scope.Connection;
    public NpgsqlTransaction Transaction => _scope.Transaction;
    ...
    public async Task SyncTableAsync<T>(CancellationToken ct = default) where T : class
    {
        ...
        if (!exists)
        {
            ...
            _logger?.Info($"[PostgreSQL] (migration) created table `{tableName}`");
            return;
        }
        ...
        foreach (var stmt in alters)
        {
            ...
            _logger?.Debug($"[PostgreSQL] (migration) {stmt}");
        }
    }
    ...
    private NpgsqlCommand NewCommand(string sql, IReadOnlyDictionary<string, object?>? parameters)
    {
