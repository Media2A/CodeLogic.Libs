...
public sealed class SchemaStateStore
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var sql = $@"
                CREATE TABLE IF NOT EXISTS {PostgreSqlDialect.Quote(StateTable)} (
                    ""TableName""     character varying(255) NOT NULL PRIMARY KEY,
                    ""SchemaCrc""     character varying(64)  NOT NULL,
                    ""Status""        character varying(20)  NOT NULL,
                    ""SyncMode""      character varying(20)  NOT NULL,
                    ""AppVersion""    character varying(50),
                    ""ModelInfo""     text,
                    ""UpdatedAt""     timestamp with time zone NOT NULL DEFAULT now(),
                    ""UpdatedByNode"" character varying(100)
                );";
            ...
            _logger?.Debug("[PostgreSQL] Schema state table ready");
            return true;
        }
        catch (Exception ex)
        {
            _logger?.Error($"[PostgreSQL] Failed to ensure schema state table: {ex.Message}", ex);
            return false;
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            _logger?.Error($"[PostgreSQL] GetStateAsync failed for `{tableName}`: {ex.Message}", ex);
            return null;
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            await _connectionManager.ExecuteWithConnectionAsync(async conn =>
            {
                await using var cmd = conn.CreateCommand();
                cmd.CommandText = $@"
                    INSERT INTO {PostgreSqlDialect.Quote(StateTable)}
                        (""TableName"", ""SchemaCrc"", ""Status"", ""SyncMode"", ""AppVersion"", ""ModelInfo"", ""UpdatedAt"", ""UpdatedByNode"")
                    VALUES (@tbl, @crc, @status, @mode, @appver, @model, now() AT TIME ZONE 'UTC', @node)
                    ON CONFLICT (""TableName"") DO UPDATE SET
                        ""SchemaCrc"" = EXCLUDED.""SchemaCrc"",
        ...
        {
            _logger?.Error($"[PostgreSQL] UpsertStateAsync failed for `{tableName}`: {ex.Message}", ex);
            return false;
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            _logger?.Error($"[PostgreSQL] RemoveStateAsync failed for `{tableName}`: {ex.Message}", ex);
            return false;
        }
    }
        ...
        CancellationToken ct = default)
    {
        try
        {
            return await _connectionManager.ExecuteWithConnectionAsync(async conn =>
            {
                ...
                cmd.CommandText = $@"
                    SELECT ""TableName"", ""SchemaCrc"", ""Status"", ""SyncMode"", ""AppVersion"", ""UpdatedAt"", ""UpdatedByNode""
                    FROM {PostgreSqlDialect.Quote(StateTable)} ORDER BY ""TableName"" ASC";
