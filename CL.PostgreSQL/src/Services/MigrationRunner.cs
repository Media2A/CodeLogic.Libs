...
public sealed class MigrationRunner
{
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var m in _migrations)
        {
            ...
            {
                _logger?.Warning(
                    $"[PostgreSQL] Migration '{MigrationId(m)}' checksum changed since it was applied — its body may have been edited.");
            }
        }
        ...
        if (pending.Count == 0)
        {
            _logger?.Debug("[PostgreSQL] No pending migrations.");
        ...
        {
            _logger?.Warning("[PostgreSQL] Migration pass skipped — another node holds the schema-sync lock.");
            return Result<MigrationRunResult>.Success(new MigrationRunResult([], 0));
        }
        ...
        foreach (var m in pending)
        {
            ...
            try
            {
                ...
                _logger?.Info($"[PostgreSQL] Applied migration '{id}': {m.Description}");
            }
            catch (Exception ex)
            {
                _logger?.Error($"[PostgreSQL] Migration '{id}' failed: {ex.Message}", ex);
                return Result<MigrationRunResult>.Failure(
                    Error.Internal("postgresql.migration_failed", $"Migration '{id}' failed", ex.Message));
            }
        }
        ...
        CancellationToken ct = default)
    {
        ...
        if (unsupported.Count > 0)
        {
            return Result<MigrationRunResult>.Failure(Error.Internal(
                "postgresql.rollback_unsupported",
        ...
        {
            return Result<MigrationRunResult>.Failure(Error.Internal(
                "postgresql.rollback_locked", "Rollback skipped — another node holds the schema-sync lock."));
        }
        ...
        foreach (var m in toRollback)
        {
            ...
            try
            {
                ...
                rolledBack.Add(id);
                _logger?.Info($"[PostgreSQL] Rolled back migration '{id}'");
            }
            catch (Exception ex)
            {
                _logger?.Error($"[PostgreSQL] Rollback of '{id}' failed: {ex.Message}", ex);
                return Result<MigrationRunResult>.Failure(
                    Error.Internal("postgresql.rollback_failed", $"Rollback of '{id}' failed", ex.Message));
            }
        }
