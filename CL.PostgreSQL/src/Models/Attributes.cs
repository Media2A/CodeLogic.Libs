...
public sealed class TableAttribute : Attribute
{
    ...
    /// <summary>
    /// PostgreSQL schema the table lives in. Defaults to <c>public</c> when null.
    /// Unlike MySQL, PostgreSQL schemas are namespaces inside one database, so every
    /// generated statement is schema-qualified.
    /// </summary>
    public string? Schema { get; set; }

    /// <summary>
    /// Table access method. PostgreSQL has no pluggable storage engines in the MySQL
    /// sense; leave this at <see cref="TableAccessMethod.Heap"/> unless an extension
    /// provides another, in which case set <see cref="AccessMethodName"/> too.
    /// </summary>
    public TableAccessMethod AccessMethod { get; set; } = TableAccessMethod.Heap;

    /// <summary>Access method name, used when <see cref="AccessMethod"/> is Custom.</summary>
    public string? AccessMethodName { get; set; }

    /// <summary>
    /// Column collation applied to text columns on this table (e.g. "en_US.utf8", or
    /// a non-deterministic ICU collation for case-insensitive comparison). Optional.
    /// Character sets are a database-wide property in PostgreSQL, not a table one, so
    /// there is no per-table charset knob.
    /// </summary>
    public string? Collation { get; set; }

    /// <summary>
    /// Table comment. Emitted as a separate <c>COMMENT ON TABLE</c> statement, since
    /// PostgreSQL has no inline table-comment clause.
    /// </summary>
    public string? Comment { get; set; }

    /// <summary>Whether the table is UNLOGGED (faster writes, not crash-safe, not replicated).</summary>
...
public sealed class ColumnAttribute : Attribute
{
    ...
    /// <summary>
    /// Former column name, used by schema sync to <c>RENAME COLUMN</c> (rename in place,
    ...
    /// <summary>
    /// Whether the database generates this column's value. Renders as
    /// <c>GENERATED BY DEFAULT AS IDENTITY</c>, the standard replacement for the older
    /// <c>serial</c> pseudo-types.
    /// </summary>
    ...
    /// <summary>
    /// Keeps this column at the current timestamp on every UPDATE. PostgreSQL has no
    /// <c>ON UPDATE CURRENT_TIMESTAMP</c> column clause, so schema sync creates a
    /// <c>BEFORE UPDATE</c> row trigger named <c>trg_{table}_{column}_touch</c> that sets it.
    /// </summary>
