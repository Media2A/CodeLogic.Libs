...
{
    private readonly ConcurrentDictionary<string, Func<NpgsqlDataReader, T>> _byShape = new();
    ...
    /// </summary>
    public Func<NpgsqlDataReader, T> CompileForReader(NpgsqlDataReader reader)
    {
        var shape = BuildShapeKey(reader);
        return _byShape.GetOrAdd(shape, _ => BuildMaterializer(reader));
    }

    private static string BuildShapeKey(NpgsqlDataReader reader)
    {
    ...
    private static Func<NpgsqlDataReader, T> BuildMaterializer(NpgsqlDataReader reader)
    {
