...
internal sealed class CursorPagingException : Exception
{
    ...
    public CursorPagingException(string message, string errorCode = "postgresql.cursor_paging_invalid", Exception? inner = null)
        : base(message, inner)
    {
        ErrorCode = errorCode;
    }
...
internal static class CursorPagination
{
        ...
        IReadOnlyList<object?> values)
    {
        if (orders.Count != values.Count)
            throw new CursorPagingException("The cursor value count does not match its ordering.", "postgresql.invalid_cursor");
        ...
        for (var i = 0; i < orders.Count; i++)
        {
            ...
            for (var prefix = 0; prefix < i; prefix++)
                terms.Add($"{PostgreSqlDialect.Quote(orders[prefix].Column.ColumnName)} " +
                          $"{PostgreSqlDialect.NullSafeEquals} @cursor_{prefix}");

            var column = $"{PostgreSqlDialect.Quote(orders[i].Column.ColumnName)}";
            var value = values[i];
            string? comparison;
...
internal static class CursorTokenCodec
{
    ...
    private static CursorPagingException Invalid(string message, Exception? inner = null) =>
        new(message, "postgresql.invalid_cursor", inner);
