...
///   <item>a compiled <c>Func&lt;NpgsqlDataReader, TResult&gt;</c> that pulls values out of
...
internal static class ProjectionCompiler
{
    ...
    public readonly struct Compiled<T, TResult>
    {
        ...
        public Func<NpgsqlDataReader, TResult> Materializer { get; init; }
    }
    ...
    {
        var selectList = string.Join(", ", columns.Select(c => $"{c.Sql} AS {PostgreSqlDialect.Quote(c.Alias)}"));
    ...
    private static Compiled<T, TResult> Build<T, TResult>(Expression<Func<T, TResult>> projection)
    {
        ...
        var selectList = string.Join(", ",
            columns.Select(c => $"{c.Sql} AS {PostgreSqlDialect.Quote(c.Alias)}"));
    ...
    /// <summary>
    /// Translate a single column-level expression into a PostgreSQL SQL fragment and carry
        ...
        switch (expr)
        {
            case MemberExpression m when m.Expression is ParameterExpression:
            {
                ...
                var clrType = (prop as PropertyInfo)?.PropertyType
                              ...
                              ?? typeof(object);
                return ($"{PostgreSqlDialect.Quote(colName)}", clrType);
            }
    ...
    /// <summary>
    /// Build a delegate that reads aliased columns from <see cref="NpgsqlDataReader"/> and
    ...
    /// </summary>
    private static Func<NpgsqlDataReader, TResult> BuildMaterializer<TResult>(
        Expression body,
        List<(string Sql, string Alias, Type CLRType)> columns)
    {
