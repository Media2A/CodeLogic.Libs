...
/// <summary>Published when a PostgreSQL connection is successfully opened.</summary>
...
/// <summary>Published when a PostgreSQL connection is closed.</summary>
public record DatabaseDisconnectedEvent(
    string ConnectionId,
    DateTime DisconnectedAt) : IEvent;

/// <summary>
/// Published after a table is synchronized (created or altered). Unlike the MySQL
/// equivalent this carries the schema as well as the table, since PostgreSQL table
/// names are only unique within a schema.
/// </summary>
...
/// <summary>
/// Published when a query exceeds the configured slow-query threshold. When
/// <c>CaptureExplainOnSlowQuery</c> is enabled the plan is attached as JSON.
/// </summary>
